...
function tripleLabel(triple: TripleRecord | undefined) {
  if (!triple) return "n/a";
  return `${triple.DisplayHead ?? triple.Head} [${triple.DisplayRelation ?? triple.Relation}] ${triple.DisplayTail ?? triple.Tail}`;
}
...
/**
 * Collapsible right-side drawer that overlays the graph canvas.
 * Does NOT consume graph width — the graph always gets 100% of its container.
 */
export function GraphInspectorDrawer({
  graph,
  selectedNode,
  selectedEdge,
  connectedEdges,
  ...
  onClose,
  onClear,
}: GraphInspectorDrawerProps) {
  return (
    <aside
      className="inspector-drawer absolute right-0 top-0 z-20 h-full w-[min(24rem,90vw)] overflow-auto border-l border-border bg-surface/95 p-5 shadow-2xl backdrop-blur-md sm:w-[22rem]"
      data-open={open}
    >
      ...
      {!selectedNode && !selectedEdge ? (
        <div className="mt-6 space-y-4 text-sm leading-6 text-muted">
          <p>
            Click an entity, component, cluster, candidate, or relation to inspect provenance
            without crowding the canvas.
          </p>
          <div className="rounded-card border border-border bg-bg p-4">
            Zoomed out shows structural summaries. Hover or select edges to reveal labels and
            supporting paths.
          </div>
        </div>
      ) : null}
        ...
        <div className="mt-6 space-y-5">
          <div className="rounded-card border border-border bg-bg p-4">
            ...
            {selectedNode.warning ? (
              <div className="mt-3 rounded-lg bg-amber/10 px-3 py-2 text-sm text-amber">
                {selectedNode.warning}
              </div>
            ) : null}
          </div>
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {(selectedNode.sample_relations ?? []).length ? (
                selectedNode.sample_relations?.map((rel) => (
                  <span
                    key={rel}
                    className="rounded-full border border-border bg-bg px-3 py-1.5 text-xs text-muted"
                  >
                    {rel}
                  </span>
                ))
              ) : (
                <span className="text-sm text-muted">No sample relations returned.</span>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 space-y-2">
              ...
              {!connectedEdges.length ? (
                <div className="text-sm text-muted">No visible connected edges.</div>
              ) : null}
            </div>
          </div>
        </div>
      ...
      {selectedEdge ? (
        <div className="mt-6 space-y-5">
          <div className="rounded-card border border-border bg-bg p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-1 break-words text-lg font-semibold text-ink">{selectedEdge.label}</div>
              </div>
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {(selectedEdge.provenance?.source_heads ?? []).length ? (
                ...
                <span className="text-sm text-muted">No source heads returned.</span>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 space-y-2">
              {(selectedEdge.provenance?.source_triples ?? []).slice(0, 6).map((triple, i) => (
                <div
                  key={`${triple.Head}-${triple.Relation}-${triple.Tail}-${i}`}
                  className="rounded-lg border border-border bg-bg px-3 py-2 text-sm text-muted"
                >
                  {tripleLabel(triple)}
                </div>
              ...
              {!(selectedEdge.provenance?.source_triples ?? []).length ? (
                <div className="text-sm text-muted">No source triples returned.</div>
              ) : null}
            </div>
          </div>
        </div>
